...
App.controller('ManageStationsCtrl', [
  '$scope',
  '$localStorage',
  '$timeout',
  '$http',
  ...
  function($scope, $localStorage, $timeout, $http, $window, $mdDialog) {
    ...
    function DialogController($scope, $http, $mdDialog) {
      ...
      $http.get('/api/sensors/types').then(
        function(response) {
...
App.controller('SettingsCtrl', [
  ...
  function(
    ...
    $scope.configure = function(db) {
      ...
      $http.post('/api/db/connect', db).then(
        function(response) {
...
App.controller('MapperCtrl', [
  ...
  function($scope, $localStorage, $window, $mdDialog) {
      ...
      if (data[pos] == 'Object') {
        ...
        if (obj.hasOwnProperty(data[pos + 1])) {
          if (data[pos + 2] == 'Array') {
            ...
            obj[data[pos + 1]].push(
              recurse(pos + 3, data, obj[data[pos + 1]], obj),
            );
          }
          return obj;
        } else {
    ...
    function DialogController($scope, $mdDialog, $http, type) {
      ...
      $http.get('/assets/json/mapper/' + json + '.json').then(
        function(response) {
    ...
    function SettingsController($scope, $mdDialog, $http, type) {
      ...
      $http.get('/assets/json/mapper/' + json + '.json').then(
        function(response) {
          ...
          for (var i = 0; i < my_array.length; ++i) {
            if (my_array[i] != 'None')
              $scope.mapper[$scope.type][my_array[i]] = 'None';
          }
      ...
      $scope.setup = function() {
        ...
        $http.post('/api/fs/write', $scope.content).then(
          function(response) {
    ...
    var initChatWindows = function(customHeight) {
      if (customHeight) {
        ...
        var cHeight =
          ...
          lFooter.outerHeight() -
          cHead.outerHeight() -
          (parseInt(cContainer.css('padding-top')) +
            parseInt(cContainer.css('padding-bottom')));
...
App.controller('TablesDatatablesCtrl', [
  '$scope',
  '$localStorage',
  '$window',
  function($scope, $localStorage, $window) {
...
App.controller('FormsPickersMoreCtrl', [
  '$scope',
  '$localStorage',
  '$window',
  function($scope, $localStorage, $window) {
...
App.controller('FormsEditorsCtrl', [
  '$scope',
  '$localStorage',
  '$window',
  function($scope, $localStorage, $window) {
...
App.controller('FormsValidationCtrl', [
  '$scope',
  '$localStorage',
  '$window',
  function($scope, $localStorage, $window) {
...
App.controller('FormsWizardCtrl', [
  '$scope',
  '$localStorage',
  '$window',
  function($scope, $localStorage, $window) {
  ...
  function($scope, $localStorage, $window) {
    ...
    var initChartsFlot = function() {
      ...
      flotLines.bind('plothover', function(event, pos, item) {
        if (item) {
          if (previousPoint !== item.dataIndex) {
            ...
            jQuery(
              '<div class="js-flot-tooltip flot-tooltip">' + ttlabel + '</div>',
            )
      ...
      function getRandomData() {
        ...
        var res = [];
        for (var i = 0; i < dataLive.length; ++i) res.push([i, dataLive[i]]);
      ...
      var chartLive = jQuery.plot(
        flotLive, // Init live chart
        [{data: getRandomData()}],
        {
          series: {
            shadowSize: 0,
...
App.controller('CompSyntaxHighlightingCtrl', [
  '$scope',
  '$localStorage',
  '$window',
  function($scope, $localStorage, $window) {
    ...
    var initRating = function() {
      ...
      jQuery('.js-rating').each(function() {
        ...
        ratingEl.raty({
          ...
          targetScore: ratingEl.data('target-score')
            ? ratingEl.data('target-score')
            : false,
          precision: ratingEl.data('precision')
            ? ratingEl.data('precision')
            : false,
          cancelOff: ratingEl.data('cancel-off')
            ? ratingEl.data('cancel-off')
            : 'fa fa-fw fa-times text-danger',
          cancelOn: ratingEl.data('cancel-on')
            ? ratingEl.data('cancel-on')
            : 'fa fa-fw fa-times',
          starHalf: ratingEl.data('star-half')
            ? ratingEl.data('star-half')
            : 'fa fa-fw fa-star-half-o text-warning',
          starOff: ratingEl.data('star-off')
            ? ratingEl.data('star-off')
            : 'fa fa-fw fa-star text-gray',
          starOn: ratingEl.data('star-on')
            ? ratingEl.data('star-on')
...
App.controller('CompMapsGoogleCtrl', [
  '$scope',
  '$localStorage',
  '$window',
  function($scope, $localStorage, $window) {
    ...
    var initMapSearch = function() {
      ...
      jQuery('.js-form-search').on('submit', function() {
        GMaps.geocode({
          address: jQuery('.js-search-address')
            .val()
            .trim(),
    ...
    var initMapOverlay = function() {
      new GMaps({
        ...
        lng: -122.4376,
        content:
          '<div class="alert alert-danger alert-dismissable"><h4 class="push-15">Overlay Message</h4><p class="push-10">You can overlay messages on your maps!</p></div>',
      });
    };
      ...
      GMaps.geolocate({
        success: function(position) {
          ...
          gmapGeolocation.addMarker({
            ...
            infoWindow: {
              content:
                '<div class="text-success"><i class="fa fa-map-marker"></i> <strong>Your location!</strong></div>',
            },
          });
        },
...
App.controller('CompMapsGoogleFullCtrl', [
  '$scope',
  '$localStorage',
  '$window',
  function($scope, $localStorage, $window) {
...
App.controller('CompMapsVectorCtrl', [
  '$scope',
  '$localStorage',
  '$window',
  function($scope, $localStorage, $window) {
