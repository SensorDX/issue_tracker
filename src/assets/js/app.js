...
App.config([
  ...
  function($stateProvider, $urlRouterProvider) {
    $urlRouterProvider.otherwise('/faultinbox');
    $stateProvider
      .state('faultinbox', {
        url: '/faultinbox',
  ...
  return {
    ...
    uiHandleColorTheme: function(themeName) {
      ...
      if (themeName) {
        if (
          colorTheme.length &&
          colorTheme.prop('href') !==
            'assets/css/themes/' + themeName + '.min.css'
        ) {
          jQuery('#css-theme').prop(
            'href',
            'assets/css/themes/' + themeName + '.min.css',
          );
        } else if (!colorTheme.length) {
          jQuery('#css-main').after(
            '<link rel="stylesheet" id="css-theme" href="assets/css/themes/' +
              themeName +
              '.min.css">',
          );
        }
    ...
    uiHandleHeader: function() {
      ...
      if (
        lPage.hasClass('header-navbar-fixed') &&
        lPage.hasClass('header-navbar-transparent')
      ) {
    ...
    uiHandleScroll: function(mode) {
      ...
      if (mode === 'init') {
        ...
        if (
          windowW > 991 &&
          lPage.hasClass('side-scroll') &&
          (lSidebar.length || lSideOverlay.length)
        ) {
          // If sidebar exists
          if (lSidebar.length) {
            ...
            if (
              lSidebarScroll.length &&
              !lSidebarScroll.parent('.slimScrollDiv').length
            ) {
          ...
          if (lSideOverlay.length) {
            ...
            if (
              lSideOverlayScroll.length &&
              !lSideOverlayScroll.parent('.slimScrollDiv').length
            ) {
      ...
      if (mode === 'init') {
        ...
        switch (button.data('action')) {
          case 'fullscreen_toggle':
            button.html(
              '<i class="' +
                (button.closest('.block').hasClass('block-opt-fullscreen')
          ...
          case 'content_toggle':
            button.html(
              '<i class="' +
                (button.closest('.block').hasClass('block-opt-hidden')
                  ? iconContentActive
                  : iconContent) +
                '"></i>',
            );
...
App.controller('AppCtrl', [
  ...
  function($scope, $localStorage, $window, $location, $log) {
    ...
    $scope.$watch(
      'oneui.settings.activeColorTheme',
      function() {
        // Handle Color Theme
        $scope.helpers.uiHandleColorTheme(
          $scope.oneui.settings.activeColorTheme,
        );
      },
      true,
    );
...
App.controller('SideOverlayCtrl', [
  '$scope',
  '$localStorage',
  '$window',
  ...
  function($http) {
    ...
    function GetUsers(fields = [], roles = ['agent,manager']) {
      if (fields && fields.length > 0) {
        return $http.get(
          'api/users?role=' + roles.join(',') + '&&fields=' + fields.join(','),
        );
      }
